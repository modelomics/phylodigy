...
def build_modelome_tree(
    ...
    *,
    ...
    selected = config if isinstance(config, LineageAnalysisConfig) else (
        LineageAnalysisConfig.from_mapping(config) if config is not None
    ...
    if len(genomes) >= 2:
        result = infer_lineage_network(genomes, config=selected)
        result.pop("digest")
    ...
    result["modelome"] = {
        ...
        "integrity": "verified_bundle" if source["manifest"] is not None else "unverified_file",
        "entries": entries,
